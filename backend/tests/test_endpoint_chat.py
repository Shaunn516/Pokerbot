...
def test_api_poker_chat_accepts_learn_request(monkeypatch):
    async def fake_call_deepseek(messages, settings):
        joined = "\n".join(message["content"] for message in messages)
        assert "Detected intent: lesson_explanation" in joined
        assert "Current topic: position" in joined
        return "Simple Explanation: Position is when you act.\nExample: BTN acts late.\nBeginner Tip: Later is easier."
    ...
    response = client.post(
        ...
        json={
            ...
            "language": "en",
            "mode": "learn",
            "lessonState": {"lessonId": "basics", "currentTopic": "position"},
        },
    )
